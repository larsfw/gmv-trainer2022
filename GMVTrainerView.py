...
class TrainerView(QtWidgets.QMainWindow):
    ...
    def saveDialog(self, pfad):
        return QtWidgets.QFileDialog.getSaveFileName(self,
    ...
    def zeigeUebersicht(self, uebersicht_df):
        ...
        for aufgabe in uebersichtArray:
            if str(aufgabe[2]) == 'nan':
                ...
                aufgabe [3] = ''
            self.tableWidget_uebersicht.setItem(row, 0, QtWidgets.QTableWidgetItem(aufgabe[0]))
            self.tableWidget_uebersicht.setItem(row, 1, QtWidgets.QTableWidgetItem(aufgabe[1]))
            self.tableWidget_uebersicht.setItem(row, 2, QtWidgets.QTableWidgetItem(aufgabe[2]))
            self.tableWidget_uebersicht.setItem(row, 3, QtWidgets.QTableWidgetItem(aufgabe[3]))
            if aufgabe[2] == 'vollständig':
                anzahlVollstaendig += 1
                for i in range (0, 4):
                    self.tableWidget_uebersicht.item(row, i).setBackground(QtGui.QColor(20, 200, 20))
                    i += 1
            if aufgabe[2] == 'unbearbeitet':
                for i in range (0, 4):
                    self.tableWidget_uebersicht.item(row, i).setBackground(QtGui.QColor(200, 200, 200))
                    i += 1
            if aufgabe[2] == 'bearbeitet':
                for i in range (0, 4):
                    self.tableWidget_uebersicht.item(row, i).setBackground(QtGui.QColor(200, 190, 0))
                    i += 1
            row += 1
    ...
    def zeigeQuantis(self, aktuelleQuantis_df):
        ...
        for quanti in quantisArray:
            self.tableWidget_quantis.setItem(row, 0, QtWidgets.QTableWidgetItem(quanti[0]))
            self.tableWidget_quantis.setItem(row, 1, QtWidgets.QTableWidgetItem(str(quanti[1])))
            self.tableWidget_quantis.setItem(row, 2, QtWidgets.QTableWidgetItem(str(quanti[2])))
            ...
            if quanti[1] == quanti[2]:
                for i in range (0, 3):
                    self.tableWidget_quantis.item(row, i).setBackground(QtGui.QColor(20, 200, 20))
                    i += 1
            if quanti[1] == 0 and quanti[2] == 0:
                for i in range (0, 3):
                    self.tableWidget_quantis.item(row, i).setBackground(QtGui.QColor(200, 200, 200))
                    i += 1
            if (quanti[1] != 0 or quanti[2] != 0) and quanti[1] != quanti[2]:
                for i in range (0, 3):
                    self.tableWidget_quantis.item(row, i).setBackground(QtGui.QColor(200, 190, 0))
                    i += 1
            row += 1
    ...
    def zeigeMerkmale(self, aktuelleMerkmale_df):
        ...
        for merkmal in merkmaleArray:
            if merkmal[2] == True:
                text_merkmale += f'<span style=" font-size:xx-large;">✔</span>  {merkmal[0]}<br><br>'
            else:
                text_merkmale += f'<span style=" font-size:xx-large;">𐄂</span>  {merkmal[0]}<br><br>'
    ...
    def zeigeMLFenster(self):
        ...
        msgBox.setWindowTitle('Fehler')
        msgBox.setIcon(3)
        msgBox.setText(f'Die Musterlösung (ML) konnte nicht geladen werden.\n\nErneut versuchen?')
        msgBox.setStandardButtons(QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
        msgBox.exec()
        if (msgBox.result() == QtWidgets.QMessageBox.Yes):
            return True
