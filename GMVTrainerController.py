...
class TrainerController():
    ...
    def connectSignals(self):
        self.view.pushButton_speicherLaden.clicked.connect(self.oeffneSpeicherstand_clicked)
        self.view.pushButton_ml.clicked.connect(self.oeffneML_clicked)
        self.view.pushButton_kp.clicked.connect(self.oeffneKP)
        self.view.pushButton_speichern.clicked.connect(lambda:self.speichern(self.view.comboBox_aufgaben.currentIndex()))
        self.view.comboBox_aufgaben.currentIndexChanged.connect(lambda:self.aendereAufgabe(self.view.comboBox_aufgaben.currentIndex()))
    ...
    def uebersichtLaden(self):
        try:
            ...
            self.view.zeigeFehlerFenster(f'Die Übersicht und/oder Aufgaben konnten nicht geladen werden.\n\nBitte lade erneut die aktuelle Musterlösung (ML) und/oder deinen Speicherstand.\n\nError: {e}')
    ...
    def oeffneSpeicherstand(self):
        try:
            ...
            self.view.zeigeFehlerFenster(f'Der Speicherstand konnte nicht geladen werden.\n\nBitte versuche es erneut.\n\nError: {e}')
    ...
    def oeffneKP(self):
        try:
            ...
            self.model.ladeKPundAbgleichML(filename[0], self.view.comboBox_aufgaben.currentIndex())
            self.view.zeigeText(self.view.textEdit_kp, self.model.kp_pfad)
        except Exception as e:
            self.view.zeigeFehlerFenster(f'Das Konstruktionsprotokoll konnte nicht geladen werden oder nicht mit der Musterlösung abgeglichen werden.\n\nError: {e}')
        
        try:
            self.view.zeigeMerkmale(self.model.aktuelleMerkmale_df)
            self.view.aktiviereSpeichern(self.model.speichernAktiv)
        except Exception as e:
            self.view.zeigeFehlerFenster(f'Die qualitativen Merkmale konnten nicht ausgewertet werden.\n\nError: {e}')
        
        try:
            self.view.zeigeQuantis(self.model.aktuelleQuantis_df)
        except Exception as e:
            self.view.zeigeFehlerFenster(f'Die quantitativen Merkmale konnten nicht ausgewertet werden.\n\nError: {e}')
        ...
        try:
            if index != -1:
                if 'True' in str(self.model.aktuelleMerkmale_df['erfüllt']):
                    ...
                    self.view.zeigeInfoFenster(f'Aufgabe {nr} erfolgreich gespeichert.\n\nStatus: {status}\n\nZeitstempel: {timestamp}')
                    self.view.comboBox_aufgaben.setCurrentIndex(-1)
                    self.view.aktiviereKPLaden(-1)
                    self.view.aktiviereSpeichern(False)
                    self.view.zeigeText(self.view.label_qualis, '<html><head/><body><p><span style=" font-size:12pt; font-weight:600;">Qualitative Merkmale</span></p></body></html>')
                    self.view.tableWidget_quantis.clear()
                    self.view.tableWidget_quantis.setHorizontalHeaderLabels(['Element', 'Deine Anzahl','ML'])
