...
class TrainerModel():
    ...
    def __init__(self):
        ...
        self.element_list = {"Punkt", "Strecke", "Gerade", "Vektor", "Ebene", "Dreieck", "Viereck", "Fünfeck", "Sechseck", "Vieleck"}
        self.kp_pfad = ''
        self.ml_pfad = ''
        self.speicher_pfad = ''
    ...
    def ladeML(self, pfad):
        if pfad[-4:] == '.xml':
            ...
            if not glob.glob(pfad + "/ML*.xml"):
                raise LookupError('Es konnte keine Musterlösungsdatei (ML) gefunden werden.')
            else:
        ...
        for aufgabe in aufgaben_root.iter('aufgabe'):
            ...
            i += 1
            new_row = {'Nr' : id, 'Titel' : titel , 'kp' : tostring(kp), 'merkmale' : merkmale}
    ...
    def erstelleSpeicherstand(self, pfad):
        ...
        i = 0
        while i < len(a_df):
            ET.SubElement(root, "status", attrib={'aufg_id':a_df['Nr'][i],'code':'unbearbeitet','datum':''})
            i += 1
    ...
    def speichernXML(self, pfad):
        ...
        i = 0
        while i < len(u_df):
            ET.SubElement(root, "status", attrib={'aufg_id':u_df['Nr'][i],'code':u_df['Status'][i],'datum':u_df['Datum'][i]})
            i += 1
        xmlstr = ET.tostring(root)
        dom = xml.dom.minidom.parseString(xmlstr)
        pretty_xmlstr = dom.toprettyxml()
    ...
    def erstelleUebersichtDf(self):
        self.uebersicht_df = pd.merge(self.aufgaben_df, self.speicher_df, how='left').drop(columns='kp').drop(columns='merkmale')

    def erstelleUebersichtList(self):
        i = 0
        uebersicht_list = self.aufgaben_list
        while i < len(uebersicht_list):
            uebersicht_list[i] += " (" + self.speicher_df['Status'][i] + ((" am " + self.speicher_df['Datum'][i]) if self.speicher_df['Datum'][i] != '' else '') + ")"
            i += 1 
    ...
    def speichereAktuelleAufgabe(self, index):
        if index != -1:
            ...
            for element in self.element_list:
                new_row = {'Element' : element, 'Deine Anzahl' : 0 , 'ML' : quanti_str.count(element)}
    ...
    def ladeKPundAbgleichML(self, pfad, index):
        ...
        aktuelleQuantis_df = pd.DataFrame(columns=['Element','Deine Anzahl','ML'])
        for element in self.element_list:
            new_row = {'Element' : element, 'Deine Anzahl' : quanti_str.count(element) , 'ML' : self.aufgQuanti_str.count(element)}
            aktuelleQuantis_df = aktuelleQuantis_df.append(new_row, ignore_index=True)
        self.aktuelleQuantis_df = aktuelleQuantis_df.sort_values(by='Element')
        ...
        for merkmal in self.aufgaben_df['merkmale'][index]:
            name = merkmal.attrib['name']
            kriterium = merkmal.text
            new_row = {'Merkmal' : name, 'Kriterium' : kriterium , 'erfüllt' : True if kriterium in quali_str else False}
            aktuelleMerkmale_df = aktuelleMerkmale_df.append(new_row, ignore_index=True)
        self.aktuelleMerkmale_df = aktuelleMerkmale_df
